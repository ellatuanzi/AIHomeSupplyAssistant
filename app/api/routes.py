...
async def upload_receipt(file: list[UploadFile] = File(...)) -> dict[str, object]:
    ...
    if len(files) > 3:
        raise HTTPException(status_code=400, detail="一次最多上传 3 个文件。")
    uploads = []
    total_size = 0
    for upload in files:
        ...
        if len(data) > 3 * 1024 * 1024:
            raise HTTPException(status_code=400, detail="单个小票文件太大，请控制在 3MB 以内。")
        total_size += len(data)
        if total_size > 6 * 1024 * 1024:
            raise HTTPException(status_code=400, detail="上传文件总大小太大，请控制在 6MB 以内。")
        uploads.append(
            (

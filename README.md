...
Gmail 订单分析默认关闭。手动触发时会直接跳过，除非设置
`ENABLE_GMAIL_ORDER_ANALYSIS=true`：

```http
POST /agent/order-analysis
```

建议先使用下方小票/截图上传补录订单，避免 Gmail restricted scope 审核。
...
默认情况下，这个任务只检查 Google Sheet 中的低库存、待办和推荐状态，并让 `/chat` 页面显示最新摘要；不会读取 Gmail，也不会发送邮件。

如果新增 Gmail 读取或 Gmail 发信权限，需要重新 Google OAuth 授权，并更新 Render 的 `GOOGLE_TOKEN_JSON`。
...
A practical MVP for reducing household mental load around everyday supplies. When something is running low, a household member taps an NFC tag, uses a voice shortcut, or opens a lightweight web page. The backend records the event in Google Sheets, reviews purchase history and preferences, and shows a Chinese daily summary in the chat page. The system recommends and tracks. It does not auto-purchase anything.
...
7. The user can upload receipt or order screenshots, and the system writes purchase/order analysis to Google Sheets.
8. The chat page shows a Chinese daily summary when there is something meaningful to review.
9. The user manually reviews links and decides whether to buy. Checkout always requires human approval.

No email is sent by default. This avoids Gmail OAuth restricted-scope verification.
